...
2\) Completar los elementos faltantes para realizar la definición de una función que permite realizar el cálculo del PIB percápita dado el PIB y el total de habitantes de un país.
...
return(pib_per_capita)}  # Imprimir el resultado
...
Estas indicando `print(pib_per_capita)` fuera de la definición de la función. Incluso, estas definiendo la función dos veces. Una lo haces correctamente y la otra cometes el error que te estoy indicando. Las líneas siguientes a Ejemplo de uso las dejas comentadas y no ejecutas el llamado a la función con los correspondientes valores de `pib_total` y `poblacion`
